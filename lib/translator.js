...
DebugTranslator.prototype.setBreakpoint = function(file, line, cb) {
    ...
    else if (file && this.script) {
        // !sbp <lineno>
        line = parseInt(file) - 1;
        file = this.script;
    }
...
DebugTranslator.prototype.fixRemoteFixStdout = function(outCb, errCb, cb) {
    cb = cb || function() {};
    code = '(function() {' + 
            'process.stdout.write = (function (write) {return function(string, encoding, fd) {outCb(string, encoding); write.call(process.stdout, string, encoding, fd);}})(process.stdout.write);'+
            'process.stderr.write = (function (write) {return function(string, encoding, fd) {errCb(string, encoding);  write.call(process.stderr, string, encoding, fd);}})(process.stderr.write);'+
            '})()';
    this.evaluateAtFrame(code, this.maxFrames - 1, cb);
};
