...
DebugTranslator.prototype.removeBreakpoint = function(file, line, cb) {
    ...
    if (breakpoint || breakpoint === 0) {
        ...
        this.findFiles(file, function(err, fullPath) {
            ...
            that.client.clearbreakpoint(breakpoint, callbackModifier(cb));
            delete that.breakpoints[breakpoint];
        });
    }
...
DebugTranslator.prototype.findFiles = function(file, cb) {
    this.client.scripts(V8Debugger.NORMAL_SCRIPTS, null, true, function(res, refs, err) {
        ...
        var matches = scripts.filter(function(script) {
            ...
            console.log(file, path, path.indexOf(file));
            if (path === file || path.indexOf(file) >= 0) {
                return true;
            }
            else {
                return false;
...
DebugTranslator.prototype.fixStdout = function(cb) {
    cb = cb || function() {};
    code = '(function() {var fs = require("fs"); var f = fs.openSync("tmp","w"); process.stderr.write = process.stdout.write = (function (write) {return function(string, encoding, fd) {fs.writeSync(f, string); }})(process.stdout.write)})()';
